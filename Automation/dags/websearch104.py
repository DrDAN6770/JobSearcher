...
class eJob_search104():
    ...
    def __init__(self, filter_params, key_word):
        self.filter_params = filter_params
        self.key_word = key_word
    
    def search_job(self):
        u = requests.get(self.search_url, self.filter_params, headers=self.headers).url
        print(u)
        u = u + '&page='
        Raw_Job_list = []
        for x in range(1,101):
            try:
                ...
                raw_Job_list = s.find_all("a",class_="js-job-link")
                if not raw_Job_list:
                    print(f'第{x}頁無資料')
                    break
                Raw_Job_list.extend(raw_Job_list)
                print(f'搜尋第{x}頁')
...
def execute():
    ...
    clean_df.to_csv(f"/opt/airflow/output/JBLIST_{current_date}.csv", sep=',', index=False)